...
def Desenho_Carga(d, Carga, posicao_elementos, dicionario_cores, Barra):
    for index, row in Carga.iterrows():
        ...
        d += elm.Line().at(posicao_barra).to(posicao_carga).color(dicionario_cores[tensao_barra_conecatada])
        d += elm.GroundSignal().at(posicao_carga).theta(teta+90).label(f"{nome}\n{potencia}\n{conexao}", loc='bottom').color(dicionario_cores[tensao_barra_conecatada]).fill()