...
def gerar_relatorio_completo(caminho, resultados, Configuracoes, potencia_base, nome_caso_estudo, unidade):
    ...
    titulo = sheet.range('B2') # Define a célula onde o título será inserido
    ...
    data_atual.value = f"Data: 06/04/2026 15:53:14" # Insere a data atual # Alterar no futuro
    ...
    for index_configuracoes, row_configuracao in Configuracoes.iterrows():
        ...
        titulo_caso_estudo.font.bold = True # Deixa o título em negrito
        titulo_caso_estudo.font.size = 15 # Define o tamanho da fonte do título
        titulo_caso_estudo.api.HorizontalAlignment = -4108  # xlCenter
        titulo_caso_estudo.color = (217, 217, 217) # pintando de Cinza
        ultima_linha += 1
        ...
        sheet.range(f"B{ultima_linha}").options(index=False, header=False).value = resultados['Tensões nas Barras'][index_configuracoes]
        ultima_linha += 1
